...
func notify(a Agent, mode string) {
	...
	notifyMessage(msg, mode)
}

func notifyCompletion(a Agent, mode string) {
	msg := fmt.Sprintf("%s finished (%s:%d)", a.Name, a.Pane.Session, a.Pane.WindowIndex)
	notifyMessage(msg, mode)
}

func notifyMessage(msg, mode string) {
	switch mode {
	case "", "tmux", "both":
	...
	switch mode {
	case "desktop", "both":
		desktopNotify("tmux-sentinela", msg)
	}
}

func desktopNotify(title, body string) {
	for _, command := range desktopNotificationCommands(runtime.GOOS, title, body) {
...
func desktopNotificationCommands(goos, title, body string) []commandSpec {
	if goos == "darwin" {
		return []commandSpec{
	...
	return []commandSpec{{program: "notify-send", args: []string{"--", title, body}}}
}
